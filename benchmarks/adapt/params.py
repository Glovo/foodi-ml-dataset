...
def get_test_params(ensemble=False):
    ...
    if ensemble:
        parser.add_argument(
            "options", type=str, nargs="+", help="YAML paths to test options"
        )
    else:
        parser.add_argument("options", type=str, help="YAML path to test options")
    parser.add_argument("-local_rank", type=int, default=0)
    parser.add_argument(
        "-device", default="cuda", help="Device option to run test script"
    )
    parser.add_argument(
        "-data_split", "-s", default="dev", help="Data split to run test script"
    )
...
def get_vocab_builder_params():
    ...
    parser.add_argument(
        "-char_level", action="store_true", help="Define character or word level"
    )
    parser.add_argument(
        "-data_name", nargs="+", default=["f30k_precomp"], help="Name of dataset"
    )
