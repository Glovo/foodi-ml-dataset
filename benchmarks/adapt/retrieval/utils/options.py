        ...
        if key == "_{}__locked".format(type(self).__name__):
            ...
            if self.__locked:
                raise PermissionError(
                    "Options' dictionnary is locked and cannot be changed."
                )
            if type(val) == dict:
                val = OptionsDict(val)
                OrderedDict.__setitem__(self, key, val)
...
class Options(object):
    ...
    def __new__(
        self, path_yaml=None, arguments_callback=None, lock=False, run_parser=True
    ):
        ...
        if not Options.__instance:
            ...
            if path_yaml:
                ...
                optfile_parser.add_argument(
                    "-o", "--path_opts", type=str, required=True
                )
                self.path_yaml = optfile_parser.parse_known_args()[0].path_opts

            options_yaml = load_yaml_opts(self.path_yaml)

            if run_parser:
                fullopt_parser = Options.HelpParser(
                    formatter_class=argparse.ArgumentDefaultsHelpFormatter
                )
                fullopt_parser.add_argument(
                    "-o", "--path_opts", type=str, required=True
                )
                ...
                if arguments_callback:
                    arguments = arguments_callback(
                        Options.__instance, arguments, options_yaml
                    )
                ...
                for argname in vars(arguments):
                    ...
                    for piece in nametree[:-1]:
                        if piece in position and isinstance(
                            position[piece], collections.Mapping
                        ):
