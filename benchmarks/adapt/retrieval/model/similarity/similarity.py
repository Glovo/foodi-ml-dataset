...
class AdaptiveEmbeddingI2T(nn.Module):
    ...
    def forward(self, img_embed, cap_embed, lens, **kwargs):
        ...
        for i, img_tensor in enumerate(img_embed):
            ...
            txt_output = self.adapt_txt(value=cap_embed,
                                        query=img_vector)  # torch.Size([1, 2048, 200]))
            ...
            txt_output = txt_output[:, :, :lens[
                0]]  # Getting only the txt output until the length of the caption (NO PADDING)
