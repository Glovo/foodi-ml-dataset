...
class Tokenizer(object):
    ...
    def __init__(
        self, vocab_path=None, char_level=False, maxlen=None, download_tokenizer=False
    ):
    ...
    def fit(self, sentences, threshold=4):
        logger.debug(f"Fit word vocab on {len(sentences)} and t={threshold}")
        counter = Counter()
    ...
    def save(self, outpath):
        ...
        logger.info(f"Vocab stored into {outpath} with {len(self.vocab)} tokens.")
    ...
    def tokenize(self, sentence):
        ...
        if self.char_level:
            tokens = " ".join(tokens)
        tokens = (
            [self.vocab("<start>")] + self.tokens_to_int(tokens) + [self.vocab("<end>")]
        )
    ...
    def decode_tokens(self, tokens):
        ...
        text = join_char.join([self.vocab.get_word(token) for token in tokens])
        return text
