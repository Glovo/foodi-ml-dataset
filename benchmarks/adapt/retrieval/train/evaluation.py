...
def predict_loader_smart(model, data_loader, device):
    ...
    for batch in pbar_fn(data_loader):
        ...
        if img_embs is None:
            if len(img_emb.shape) == 3:
                is_tensor = True
                img_embs = np.zeros((max_samples_eval, img_emb.size(1)),
                                    dtype=np.float)
                cap_embs = np.zeros((max_samples_eval, cap_emb.size(2)),
                                    dtype=np.float)
            else:
                is_tensor = False
                img_embs = np.zeros((max_samples_eval, img_emb.size(1)),
                                    dtype=np.float)
                cap_embs = np.zeros((max_samples_eval, cap_emb.size(1)),
                                    dtype=np.float)
        ...
        cap_emb = cap_emb.permute(0, 2, 1)[...,
                  :34]  # To replicate behaviour of line #230 of similarity.py
