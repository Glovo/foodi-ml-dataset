...
def load_and_filter_file(file_path):
    ...
    print(result)
    result_filtered = {
        k.split("/")[1]: v for k, v in result.items() if k.split("/")[1] in metrics
    }

    res_line = "\t".join([f"{result_filtered[metric]:>4.2f}" for metric in metrics])
    _file = "/".join(file_path.split("/")[-3:])
    print(f"{_file:60s}\t{res_line}")
...
def load_and_filter_files(file_path):
    ...
    for data_name, vals in result_filtered.items():
        ...
        res_line = "\t".join([f"{vals[metric]:>4.2f}" for metric in metrics])
        # print(_file, data_name, res_line)

        print(f"{_file:55s}\t{data_name:20s}\t{res_line}")
